...
local RunThon = io.open("RunThon", 'w')
RunThon:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
lua5.3 RunThon.lua
...
os.execute('chmod +x RunThon;chmod +x RunThon;./RunThon')
end
...
RunThon = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..RunThon)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=RunThon,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(RunThon.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(RunThon) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(RunThon.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(RunThon) then
return false
end
if Redis:get(RunThon.."chsource") then
chsource = Redis:get(RunThon.."chsource")
else
chsource = "BxxBxxL"
end
...
local Info_User = Redis:get(RunThon.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(RunThon..'TheRunThon:BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'↯︙️ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(RunThon..'TheRunThon:BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'↯︙️ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "‹ الغاء الامر ›" then
Redis:del(RunThon..msg.sender.user_id.."bottoken")
Redis:del(RunThon..msg.sender.user_id.."botuser")
Redis:del(RunThon..msg.sender.user_id.."make:bot")
...
Redis:sadd(RunThon.."screen:on",screen)
end
local list = Redis:smembers(RunThon..'TheRunThon:screen:on')
...
Redis:set(RunThon..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"↯︙️ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(RunThon..msg.sender.user_id.."stop:bot") then
Redis:del(RunThon..msg.sender.user_id.."stop:bot")
Redis:del(RunThon.."screen:on")
Redis:del(RunThon.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(RunThon..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"↯︙️ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(RunThon.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(RunThon.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(RunThon.."chfalse")
Redis:set(RunThon.."chfalse",ch)
Redis:del(RunThon.."ch:admin")
Redis:set(RunThon.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'↯︙️ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "‹ تفعيل الاشتراك الاجباري ›" then
Redis:set(RunThon.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(RunThon..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(RunThon..msg.sender.user_id.."bottoken")
local botuser = Redis:get(RunThon..msg.sender.user_id.."botuser")
...
Redis:sadd(RunThon.."userbots",botuser)
Redis:del(RunThon..msg.sender.user_id.."bottoken")
Redis:del(RunThon..msg.sender.user_id.."botuser")
Redis:del(RunThon..msg.sender.user_id.."make:bot")
...
if text and Redis:get(RunThon..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(RunThon.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n↯︙️ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(RunThon..msg.sender.user_id.."botuser",botuser)
Redis:set(RunThon..msg.sender.user_id.."bottoken",text)
Redis:set(RunThon..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(RunThon..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n↯︙️ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(RunThon.."all:texting") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon.."all:texting")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(RunThon.."3z:gp",text)
Redis:del(RunThon.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(RunThon.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(RunThon.."3z:gp")
Redis:del(RunThon.."all:texting")
send(msg.chat_id,msg.id,"↯︙️ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(RunThon.."all:texting:pv") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon.."all:texting:pv")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(RunThon.."eza3a:pv",text)
Redis:del(RunThon.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(RunThon.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(RunThon.."eza3a:pv")
Redis:del(RunThon.."all:texting:pv")
send(msg.chat_id,msg.id,"↯︙️ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "‹ اذاعة عام للمجموعات ›" then
Redis:set(RunThon.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "‹ اذاعة عام للمشتركين ›" then
Redis:set(RunThon.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "‹ فحـص ›" then
Redis:del(RunThon.."All:pv:st")
Redis:del(RunThon.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(RunThon.."All:gp:st",tonumber(#list))
Redis:incrby(RunThon.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n↯︙️ احصائيات جميع البوتات المصنوعه \n ↯︙️ عدد المجموعات '..Redis:get(RunThon.."All:gp:st")..' مجموعه \n↯︙️ عدد المشتركين '..Redis:get(RunThon.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
lt = Redis:smembers(bot_id.."Num:User:Pv") 
if #list < 2 then
Redis:sadd(RunThon.."fake",botuser )
os.execute("sudo rm -fr "..botuser)
os.execute('screen -X -S '..botuser:gsub("@","")..' quit')
end
end
end
local list = Redis:smembers(RunThon..'TheRunThon:fake')
...
-------delete 
if text and Redis:get(RunThon..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon..msg.sender.user_id.."make:bot")
...
Redis:srem(RunThon.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n↯︙️ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "‹ حذف بوت ›" then
Redis:set(RunThon..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(RunThon..'TheRunThon:3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(RunThon..'TheRunThon:3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(RunThon..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء تعيين قناه ال‹ السورس ›","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(RunThon.."chsource",ch)
...
if tonumber(RunThon) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'↯︙️ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(RunThon..'TheRunThon:total',v)  
end
end
end
if text == "‹ جلب نسخة ›" then
local UsersBot = Redis:smembers(RunThon.."total")
local Get_Json = '{"BotId": '..RunThon..'TheRunThon:,'  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(RunThon..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n↯︙️ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(RunThon.."total") 
...
if text == "‹ اذاعة ›" then
Redis:set(RunThon..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"↯︙️ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(RunThon..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '‹ الغاء الامر ›' then   
Redis:del(RunThon..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(RunThon..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(RunThon.."ch:admin")
send(msg.chat_id,msg.id,'\n↯︙️ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(RunThon.."twsl") then
if msg.sender.user_id ~= tonumber(RunThon) then
if Redis:sismember(RunThon..'TheRunThon:BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(RunThon.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(RunThon.."free:bot") then
return send(msg.chat_id,msg.id,"↯︙️ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(RunThon.."total",msg.sender.user_id) then
Redis:sadd(RunThon.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"↯︙اهلا بك في مصنع بوتات الحمايه\n↯︙اصنع بوت مجاني على سورس براند\n↯︙لايحتوي على مشاكل تحديث 64bit\n↯︙قناه السورس : ‹ @BxxBxxL ›","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(RunThon..msg.sender.user_id.."make:bot") then
if text == "‹ الغاء ›" then
Redis:del(RunThon..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(RunThon..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(RunThon.."userbots",botuser)
Redis:del(RunThon..msg.sender.user_id.."make:bot")
...
if text == "‹ انشاء بوت ›" then
if Redis:get(RunThon..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"↯︙️ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(RunThon..msg.sender.user_id.."make:bot",true)
...
if text == "‹ حذف بوت ›" then
if Redis:get(RunThon..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(RunThon..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(RunThon.."userbots",botuser)
Redis:del(RunThon..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n↯︙️ تم حذف البوت بنجاح","md",true)  
else
...
----
if text == 'المطور علش' or text == '‹ المبرمج ›' or text == 'علش' then  
...
if tonumber(data.message.sender.user_id) == tonumber(RunThon) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == RunThon then
return false
end
